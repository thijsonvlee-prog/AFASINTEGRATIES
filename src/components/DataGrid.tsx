...
export function DataGrid({ data, onSort }: DataGridProps) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="rounded-2xl border border-white/20 bg-white overflow-x-auto -mx-4 md:mx-0 shadow-bento">
        <div className="min-w-[600px]">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b bg-slate-50/80">
                {columns.map((col) => (
                  <th key={col} className="px-3 py-2.5 text-left font-bold cursor-pointer hover:bg-slate-100/60 transition-colors whitespace-nowrap text-[10px] uppercase tracking-wider text-slate-400" onClick={() => handleSort(col)}>
            ...
            <tbody>
              {data.map((row, i) => (
                <tr key={i} className="border-t border-border/30 hover:bg-accent/30 transition-colors">
                  {columns.map((col) => (
                    <td key={col} className="px-3 py-2 whitespace-nowrap max-w-[200px] md:max-w-[300px] truncate text-xs md:text-sm font-mono tabular-nums">
